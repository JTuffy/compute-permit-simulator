...
|---|---|---|
| `run_single(config)` | `services.simulation_runner` | One specific config, one deterministic result |
| `run_monte_carlo(config, n_runs, store_raw)` | `services.monte_carlo` | Distribution of outcomes across seeds |
| `run_sweep(base_config, param_path, values, n_runs)` | `services.sweep` | How one parameter shifts compliance |
| `load_scenario(filename)` | `services.config_manager` | Load a JSON scenario file into `ScenarioConfig` |
| `get_param(path)` / `generate_values(param, ...)` | `schemas.sweep_params` | Look up registry param + generate sweep range |
...
1. `findings.md` — hypothesis, result, interpretation, recommended next experiment
2. `exp<n>_<name>.py` — re-runnable experiment scripts
3. `*.png` — figures generated by scripts
4. `scenarios/*.json` — draft scenario configs (promote to `scenarios/basic/` only when finalized)

**Known degenerate regime:** The default `ScenarioConfig` produces 100% compliance at all tested audit probabilities. The permit cap (20) is generous for 15 agents — no scarcity, so no reason to violate. Always start from a margin scenario (`agent_workspace/research/*/scenarios/research_margin_baseline.json`) where economic value < penalty × detection probability creates genuine agent indifference.
